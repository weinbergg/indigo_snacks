          ...
          {galleryItems.map((item, index) => (
            <Reveal key={item.caption} delay={index * 0.08} className="h-full">
              <figure className="interactive-panel group relative flex h-full flex-col overflow-hidden rounded-[1.8rem] border border-line/70 bg-white/80 p-5 sm:p-6">
                <div className="flex aspect-[4/3.45] items-center justify-center overflow-hidden rounded-[1.4rem] bg-brand-soft/50 p-3 sm:p-4">
                  <img
                    src={item.src}
                    alt={item.alt}
                    className="h-full w-full rounded-[1.15rem] object-cover transition duration-500 group-hover:scale-[1.02]"
